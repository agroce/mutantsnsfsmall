...
generating mutants to be used in this project's early phases.

{\bf CLG: TALK ABOUT COMBY HERE.}
Recent work by PI CLG \& co. develops a new representation to declaratively
transform richer syntactic structures in programs across multiple
languages~\cite{rvt-ppc}. This work demonstrates practical utility by
performing lightweight refactors in more than 10 languages (including those
targetted by {\tt universalmutator}). The associated tool, {\tt
...
nested code blocks). Current mutation testing approaches largely rely on either
sophisticated language-specific tools that fail to generalize to other
languages~\cite{?} or rudimentary textual manipulation~\cite{?} that miss the
addressable test surface. Coupling declarative syntax manipulation with
mutation testing promises to yield greater effectiveness by (a) targetting more
sophisticated properties of programs, and (b) delivering more user-accessible
tools for developing mutation transformations. Such a midway is
largely unexplored in current solutions. However, recent advances in respective
real-world tooling (i.e., in {\tt universalmutator} and {\tt comby}) suggest that
this midway is imminently feasible.
